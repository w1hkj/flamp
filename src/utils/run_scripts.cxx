...
static int process_compression(ScriptParsing *sp, SCRIPT_COMMANDS *sc)
{
	...
	btn_use_compression->do_callback();
...
static int process_proto(ScriptParsing *sp, SCRIPT_COMMANDS *sc)
{
	...
	btn_enable_tx_unproto->value(flag);
	progStatus.enable_tx_unproto = flag;
...
void script_execute(const char *filename, bool queue_flag)
{
	...
	count = 0;
	while(s_basic_modes[count]) {
		if(count > 100) break;
		count++;
	}

	// Currently each command requires code to make use of these "valid parameters"
	sp->assign_valid_parameters("MODEM", s_basic_modes, count);
	sp->assign_valid_parameters("HEADER MODEM", s_basic_modes, count);
	sp->assign_valid_parameters("HAMCAST MODEM", s_basic_modes, count);
 ...
 * \param void
 ***********************************************************/
void cb_scripts(void)
{
	std::string script_filename = "";

	script_filename.clear();

	const char *p = FSEL::select(_("Execute Script file"), "Text Files\t*.txt",
	...
	script_execute((char *) script_filename.c_str(), false);
}
...
void cb_load_tx_queue(void)
{
	...
	if(progStatus.load_from_tx_folder) {
		...
		script_execute(script_filename.c_str(), true);
	}
}
