...
#define P_RELSOL2 "%1.6f"
	...
	if(index > 99) {
		LOG_DEBUG("%s", \
			_("time_table-xx.txt files reached count limit, delete some."));
		in_use = 0;
	...
	if(fd) {
		LOG_DEBUG("%s %s", _("Table Gen filename"), filename);
		...
		do {
			LOG_DEBUG("%s, %s", _("Processing Modem"), cbo_modes->value());
			...
			LOG_DEBUG("%s %s %s %f", _("Mode"),  mode_name.c_str(), _("overhead ="), overhead);
			...
			if(mode_name.find("Olivia") != string::npos || mode_name.find("MT63") != string::npos) {
				...
				for(index = 0; index < 256; index++) {
					...
					if(index < ' ')
						LOG_DEBUG("%s \'%d\' (%X) %s %f", _("Charater"), \
						 (index & 0xff), (index & 0xff), \
						 _("Transmit time in seconds"), \
						 time_seconds);
					else
						LOG_DEBUG("%s \'%c\' %s %f", _("Charater"), \
							(index & 0xff), _("Transmit time in seconds"),  \
							time_seconds);
		...
		LOG_ERROR((const char *) "%s = %d", _("File open Error"), errno);
	}
...
double measure_tx_time(int character_to_send, double *over_head)
{
	...
	if(tx_duration.size() < 1) {
		LOG_ERROR("%s", _("No Data returned."));
		transmit_stop = true;
		return retun_val;
	}
	...
	if(count != 3) {
		LOG_ERROR("%s %d %s", _("Returned parameter error. Count ="), count, \
			_("should be 3"));
		transmit_stop = true;
		return retun_val;
	}
...
void * transmit_relay_interval(void * ptr)
{
	...
	if((vector_header_count < 1) && (vector_data_count < 1)) {
		LOG_DEBUG("%s", _("Empty xmt_vector_string"));
		return run_in_thread_destroy(thread_ptr, 3, &in_use);
	}
	...
	LOG_DEBUG("%s %d", _("vector_header_count ="), vector_header_count);

	if(progStatus.use_header_modem) {
		if(vector_header_count > 0) {
			LOG_DEBUG("%s (%s)", _("Header Modem Sent to FLDGI."), g_header_modem.c_str());
	...
	LOG_DEBUG("%s %d", _("vector_data_count ="), vector_data_count);
...
void * transmit_interval(void * ptr)
{
	...
	do {
		...
		if (!value) {
			LOG_DEBUG("%s", _("Empty xmt_buffer"));
			delete tx;
			return run_in_thread_destroy(thread_ptr, 3, &in_use);
		}
						 ...
						 std::vector<std::string> &data, TX_FLDIGI_THREAD *thread_ptr)
{
	...
	if(max_tx_time > interval_time) {
		memset(str_buffer, 0, sizeof(str_buffer));
		const char *fmtmsg =  _("Data Section Modem: %s\nRequired Block Tx time: %3.1f Mins.\nUse Faster Mode, Increase Interval Time,\nor decrease block size.");
		snprintf(str_buffer, sizeof(str_buffer) - 1, fmtmsg, \
				 modem.c_str(), max_tx_time);
		LOG_INFO("%s", str_buffer);
		return_value = false;
...
static void wait_seconds(int seconds, int millisecond_increments)
{
	...
	gettimeofday(&tp, NULL);
	current_time = (tp.tv_sec) + (tp.tv_usec * 0.000001);
	LOG_DEBUG("%s %f %s" _("SEC:"), current_time - start_time, _("DELAY"));
#endif
								 ...
								 bool queued, bool event_driven, RELAY_DATA *relay_data)
{
	...
	if(tx_thread) {
		...
		LOG_ERROR("%s", _("Thread creation error: run_in_thread()"));
	}
