...
# substitute your application name for instances of FM_MSNGR

# Variables
!define FM_MSNGR_DESCRIPTION "${FM_MSNGR_NAME} ${FM_MSNGR_VERSION}"
!define FM_MSNGR_STRING "${FM_MSNGR_NAME}-${FM_MSNGR_VERSION}"

!define PRODUCT_BINARY "${FM_MSNGR_BINARY}"
!define PRODUCT_NAME "${FM_MSNGR_NAME}"
!define PRODUCT_VERSION "${FM_MSNGR_VERSION}"
!define PRODUCT_STRING "${FM_MSNGR_STRING}"
!define PRODUCT_DESCRIPTION "${FM_MSNGR_DESCRIPTION}"
...
VIAddVersionKey FileDescription "${FM_MSNGR_DESCRIPTION} installer"
...
Section -install
    ...
    WriteRegStr HKLM "${REG_UNINSTALL_PATH}" "Publisher" "FM_MSNGR developers"
...
Section "Start Menu Shortcuts"
	...
	CreateShortCut "${SM_PATH}\${FM_MSNGR_NAME}.lnk" "$INSTDIR\${FM_MSNGR_BINARY}" "" "$INSTDIR\${FM_MSNGR_BINARY}" 0
    CreateShortCut "${SM_PATH}\Uninstall.lnk" "$INSTDIR\uninstall.exe" "" "$INSTDIR\uninstall.exe" 0
SectionEnd

Section "Desktop Shortcuts"
	CreateShortCut "$DESKTOP\${FM_MSNGR_DESCRIPTION}.lnk" "$INSTDIR\${FM_MSNGR_BINARY}" "" \
		"$INSTDIR\${FM_MSNGR_BINARY}" 0
SectionEnd

# This is unselected by default
Section /o "Quick Launch Shortcuts"
	CreateShortCut "$QUICKLAUNCH\${FM_MSNGR_DESCRIPTION}}.lnk" "$INSTDIR\${FM_MSNGR_BINARY}" "" \
		"$INSTDIR\${FM_MSNGR_BINARY}" 0
SectionEnd
...
# Remove files and uninstaller
	Delete /REBOOTOK $INSTDIR\${FM_MSNGR_BINARY}
    Delete /REBOOTOK $INSTDIR\uninstall.exe

# Remove shortcuts, if any
    Delete "${SM_PATH}\*.*"
	Delete "$DESKTOP\${FM_MSNGR_DESCRIPTION}.lnk"
	Delete "$QUICKLAUNCH\${FM_MSNGR_DESCRIPTION}.lnk"
