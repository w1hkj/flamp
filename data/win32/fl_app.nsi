...
# substitute your application name for instances of FLLNK

# Variables
!define FLLNK_DESCRIPTION "${FLLNK_NAME} ${FLLNK_VERSION}"
!define FLLNK_STRING "${FLLNK_NAME}-${FLLNK_VERSION}"

!define PRODUCT_BINARY "${FLLNK_BINARY}"
!define PRODUCT_NAME "${FLLNK_NAME}"
!define PRODUCT_VERSION "${FLLNK_VERSION}"
!define PRODUCT_STRING "${FLLNK_STRING}"
!define PRODUCT_DESCRIPTION "${FLLNK_DESCRIPTION}"
...
VIAddVersionKey FileDescription "${FLLNK_DESCRIPTION} installer"
...
Section -install
    ...
    WriteRegStr HKLM "${REG_UNINSTALL_PATH}" "Publisher" "FLLNK developers"
...
Section "Start Menu Shortcuts"
	...
	CreateShortCut "${SM_PATH}\${FLLNK_NAME}.lnk" "$INSTDIR\${FLLNK_BINARY}" "" "$INSTDIR\${FLLNK_BINARY}" 0
    CreateShortCut "${SM_PATH}\Uninstall.lnk" "$INSTDIR\uninstall.exe" "" "$INSTDIR\uninstall.exe" 0
SectionEnd

Section "Desktop Shortcuts"
	CreateShortCut "$DESKTOP\${FLLNK_DESCRIPTION}.lnk" "$INSTDIR\${FLLNK_BINARY}" "" \
		"$INSTDIR\${FLLNK_BINARY}" 0
SectionEnd

# This is unselected by default
Section /o "Quick Launch Shortcuts"
	CreateShortCut "$QUICKLAUNCH\${FLLNK_DESCRIPTION}}.lnk" "$INSTDIR\${FLLNK_BINARY}" "" \
		"$INSTDIR\${FLLNK_BINARY}" 0
SectionEnd
...
# Remove files and uninstaller
	Delete /REBOOTOK $INSTDIR\${FLLNK_BINARY}
    Delete /REBOOTOK $INSTDIR\uninstall.exe

# Remove shortcuts, if any
    Delete "${SM_PATH}\*.*"
	Delete "$DESKTOP\${FLLNK_DESCRIPTION}.lnk"
	Delete "$QUICKLAUNCH\${FLLNK_DESCRIPTION}.lnk"
